			...
			<ul class="w-nav-list level_1">
				...
				<li class="w-nav-item level_1">
					<a class="w-nav-anchor level_1" href="php/LOGOUT.PHP" style="margin-left: 210px;">
						<span class="w-nav-title">>Cerrar Sesion<</span>
						<span class="w-nav-arrow"></span>
					</a>
				</li>
			</ul>
					...
					<div>
						<?php
						 $coneccion=mysql_connect("localhost", "root","");
						    ...
						    mysql_select_db( "safemans")OR DIE ("Error: No es posible establecer la conexión");

											$query=mysql_query($con,"SELECT * FROM `detalle_usuario`");
				 ...
				 <script>
			        function Validar(nombre, apellido, rut, email, profe, fecha, estado,empresa)
			        {
			            $.ajax({
			                ...
			                type: "POST",
			                 data: "nombre0"+nombre+"&apellido="+apellido+"&rut="+rut+"&email="+email+"&profe="+profe+"&fecha="+fecha+"&estado="+estado+"&empresa="+empresa,
